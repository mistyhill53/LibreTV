			...
			try {
				...
				if (pagesToFetch > 0) {
					...
					for (let page = 2; page <= pagesToFetch + 1; page++) {
						...
						const pageUrl =
							apiBaseUrl +
							API_CONFIG.search.pagePath.replace('{query}', encodeURIComponent(query)).replace('{page}', page)
...
function playVideo(url, vod_name, sourceCode, episodeIndex = 0) {
	...
	let watchUrl = `watch.html?id=${currentEpisodes[episodeIndex].id || ''}&source=${
		sourceCode || ''
	}&url=${encodeURIComponent(url)}&index=${episodeIndex}&title=${encodeURIComponent(vod_name || '')}`
